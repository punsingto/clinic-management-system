...
export default function Sidebar() {
  ...
  return (
    <div className="flex flex-col w-64 h-full bg-white/80 backdrop-blur-md border-r border-white/20 shadow-xl">
      {/* Logo Section */}
      <div className="flex items-center space-x-3 p-6 border-b border-gray-200/50">
        ...
        <div>
          <h2 className="text-lg font-bold text-gray-900">ClinicCare</h2>
      ...
      <nav className="flex-1 px-4 py-6 space-y-2">
        {navigationItems.map((item) => {
          const active = isActive(item.href) || (item.href === '/patients' && pathname === '/');
          return (
            <Link
              key={item.name}
              href={item.href}
            ...
            >
              <span className={`mr-3 flex-shrink-0 ${active ? 'text-white' : 'text-gray-400 group-hover:text-blue-600'}`}>
                {item.icon}
              </span>
              {item.name}
              {active && (
